...
local updateBlocked = false;

local function updateUnblocker()
    updateBlocked = false;
end

local function updateBlocker()
    updateBlocked = true;
    ashita.timer.once(1.5, updateUnblocker)
end
...
local function update_party_member_texture()
    ...
    for x = 0, 5, 1 do
        ...
        if x == 0 then
            party_data[id] =
                {
                    ['buffs'] = AshitaCore:GetDataManager():GetPlayer():GetBuffs();
                }
        end
...
ashita.register_event('load', function()
	...
	for x = 0, 5, 1 do
	    ...
	    f:SetFontHeight(4 * party_buffs['scale']['y']);
	...
	elseif (id == 0x76) then
		...
		for x = 0, 4, 1 do
			...
			if (party_data[server_id] ~= nil) then
                ...
                if party_data[server_id] == AshitaCore:GetDataManager():GetParty():GetMemberServerId(0) then
                    party_data[server_id]['buffs'] = AshitaCore:GetDataManager():GetPlayer():GetBuffs();
                end
...
ashita.register_event('render', function()
    ...
    if (AshitaCore:GetDataManager():GetParty():GetAllianceParty0MemberCount() > 0) then
	    ...
	    for x = 0, 5, 1 do
			...
			if (player_zone ~= AshitaCore:GetDataManager():GetParty():GetMemberZone(x)) then
	    		f:SetVisibility(false);
	    	else
	    		for key, value in pairs(textures[x]) do
	    			local rect = RECT();
...
ashita.register_event('command', function(command, ntype)
    ...
    if args[1] == '/partybuffs' or args[1] == '/pbuffs' then
        update_party_member_texture();
        return true;
    end
    return false;
